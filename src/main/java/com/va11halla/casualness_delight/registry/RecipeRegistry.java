...
public enum RecipeRegistry {
    ...
    public static void register() {
        for (RecipeRegistry value : values()) {
            Registry.register(Registries.RECIPE_SERIALIZER, new Identifier(CasualnessDelight.MODID, value.pathName), value.serializer());
            value.type();
        }
    }
    ...
    private <T extends Recipe<? extends Inventory>> RecipeType<T> type(Class<T> clazz) {
        if (type == null) {
            type = RecipeType.register(new Identifier(CasualnessDelight.MODID, pathName).toString());
        }
